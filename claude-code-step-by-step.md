...
완전한 CREATE TABLE 스크립트와 RLS 정책, 트리거 함수를 작성해줘.
```
...
## 9단계: MCP (Model Context Protocol) 연동
...
완전한 RAG 시스템 기초를 구현해줘.
```

---

## 11단계: RFP 업로드 및 분석 시스템
...
계속해서 15-26단계까지 정리해서 추가로 제공하겠습니다.
...
운영 요건 관리 시스템을 완전히 구현해줘.
```

---

## 20단계: 업무 분야별 작업 관리 시스템
...
업무 분야별 작업 관리 시스템을 완전히 구현해줘.
```

---

## 21단계: 진행률 추적 및 리포팅 시스템
...
코드 캔버스 시스템을 완전히 구현해줘.
```

---

## 25단계: 실시간 미리보기 및 디바이스 테스트
...
실시간 미리보기 및 디바이스 테스트 시스템을 완전히 구현해줘.
```

---

## 26단계: 에셋 관리 및 최적화 시스템
...
각 단계별 프롬프트를 Claude Code에 순서대로 입력하여 개발을 진행하세요:

1. **1-3단계**: 기본 프로젝트 구조와 데이터베이스 설정
2. **4-6단계**: 인증과 기본 레이아웃 구현  
3. **7-10단계**: AI 통합과 기본 기능 구현
4. **11-14단계**: 제안 진행 모듈 완성
5. **15-18단계**: 구축 관리 모듈 완성
6. **19-22단계**: 운영 관리 모듈 완성
7. **23-26단계**: 디자인/퍼블리싱 모듈 완성

각 프롬프트를 복사하여 Claude Code에 붙여넣고 단계적으로 개발하면 완전한 엘루오 플랫폼이 구축됩니다.