...
export async function updateSession(request: NextRequest) {
  ...
  // 로그인된 사용자가 로그인 페이지에 직접 접근하면 대시보드로 리다이렉트
  // 단, 같은 도메인에서 오는 경우(폼 제출 후)는 제외
  if (user && request.nextUrl.pathname === '/auth/login' && request.method === 'GET') {
    const referer = request.headers.get('referer')
    const isFromSameDomain = referer && new URL(referer).origin === request.nextUrl.origin
    
    // 외부에서 직접 접근하거나, 다른 페이지에서 온 경우만 리다이렉트
    if (!isFromSameDomain || !referer?.includes('/auth/login')) {
      console.log('Authenticated user accessing login page directly, redirecting to dashboard')
